...
<div class="sidebar-actions">
    ...
    <ul class="icons sidebar-actions-list">
        ...
        <c:if test="${project.access == 'OPEN' || project.simpleMapAccess}">
            <li id="projectMenuPublicMap" class="analysis"><a href="${pageContext.request.contextPath}/projects/${project.id}/map">Map</a> </li>
        </c:if>
            <sec:authorize access="hasPermission(#project, 'read')">
            <c:if test="${not empty project.animals}">
                <li id="projectMenuAnalysis" class="analysis"><a href="${pageContext.request.contextPath}/projects/${project.id}/analysis">Spatial Analysis</a> </li>
